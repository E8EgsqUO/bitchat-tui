use crate::data_structures::{
    BitchatMessage, MSG_FLAG_HAS_CHANNEL, MSG_FLAG_HAS_MENTIONS, MSG_FLAG_HAS_ORIGINAL_SENDER,
    MSG_FLAG_HAS_RECIPIENT_NICKNAME, MSG_FLAG_HAS_SENDER_PEER_ID, MSG_FLAG_IS_ENCRYPTED,
    MSG_FLAG_IS_PRIVATE, MSG_FLAG_IS_RELAY,
};
...
pub fn unpad_message(data: &[u8]) -> Vec<u8> {
    ...
    debug_full_println!(
        "[PADDING] Data size: {}, padding length indicated: {}",
        data.len(),
        padding_length
    );

    // Validate PKCS#7 padding. Unpadded decrypted payloads can naturally end
    // with a small byte value, so only strip when every tail byte matches.
    ...
    {
        debug_full_println!("[PADDING] Invalid padding bytes, returning data as-is");
        return data.to_vec();
    }

    // Remove padding
    let unpadded_len = padding_start;
    debug_full_println!(
        "[PADDING] Removing {} bytes of padding, resulting size: {}",
        padding_length,
        unpadded_len
    );
...
pub fn parse_bitchat_message_payload(data: &[u8]) -> Result<BitchatMessage, &'static str> {
    ...
    if data.len() < 1 {
        return Err("Payload too short for flags");
    }
    ...
    if data.len() < offset + 8 {
        return Err("Payload too short for timestamp");
    }

    offset += 8;

    if data.len() < offset + 1 {
        return Err("Payload too short for ID length");
    }
    ...
    if data.len() < offset + id_len {
        return Err("Payload too short for ID");
    }
    ...
    if data.len() < offset + 1 {
        return Err("Payload too short for sender length");
    }
    ...
    if data.len() < offset + sender_len {
        return Err("Payload too short for sender");
    }
    ...
    if data.len() < offset + 2 {
        return Err("Payload too short for content length");
    }
    ...
    if data.len() < offset + content_len {
        return Err("Payload too short for content");
    }
    ...
    let (content, encrypted_content) = if (flags & MSG_FLAG_IS_ENCRYPTED) != 0 {
        ...
        (
            "".to_string(),
            Some(data[offset..offset + content_len].to_vec()),
        )
    } else {
        // For normal messages, parse as UTF-8 string
        (
            String::from_utf8_lossy(&data[offset..offset + content_len]).to_string(),
            None,
        )
    };
    ...
    if has_original_sender {
        if data.len() < offset + 1 {
            return Err("Payload too short for original sender length");
        }
        let orig_sender_len = data[offset] as usize;
        offset += 1;
        if data.len() < offset + orig_sender_len {
            return Err("Payload too short for original sender");
        }
    ...
    if has_recipient_nickname {
        if data.len() < offset + 1 {
            return Err("Payload too short for recipient nickname length");
        }
        let recipient_len = data[offset] as usize;
        offset += 1;
        if data.len() < offset + recipient_len {
            return Err("Payload too short for recipient nickname");
        }
    ...
    if has_sender_peer_id {
        if data.len() < offset + 1 {
            return Err("Payload too short for sender peer ID length");
        }
        let peer_id_len = data[offset] as usize;
        offset += 1;
        if data.len() < offset + peer_id_len {
            return Err("Payload too short for sender peer ID");
        }
        offset += peer_id_len;
    }
    ...
    if has_mentions {
        if data.len() < offset + 1 {
            return Err("Payload too short for mentions count");
        }
        ...
        for _ in 0..mentions_count {
            if data.len() < offset + 1 {
                return Err("Payload too short for mention length");
            }
            let mention_len = data[offset] as usize;
            offset += 1;
            if data.len() < offset + mention_len {
                return Err("Payload too short for mention");
            }
            offset += mention_len;
        }
    }
    ...
    if (flags & MSG_FLAG_HAS_CHANNEL) != 0 {
        if data.len() < offset + 1 {
            return Err("Payload too short for channel length");
        }
        ...
        if data.len() < offset + channel_len {
            return Err("Payload too short for channel");
        }
...
pub fn parse_message_payload_structured(
    data: &[u8],
) -> Result<(String, String, Option<String>, bool, String), &'static str> {
...
pub fn create_bitchat_message_payload(
    sender: &str,
    content: &str,
    channel: Option<&str>,
) -> Vec<u8> {
    ...
    let (payload, _) =
        create_bitchat_message_payload_full(sender, content, channel, false, "f453f3e0");
    payload
}

#[allow(dead_code)]
pub fn create_bitchat_message_payload_with_flags(
    sender: &str,
    content: &str,
    channel: Option<&str>,
    is_private: bool,
    ...
    let (payload, _) =
        create_bitchat_message_payload_full(sender, content, channel, is_private, "00000000");
    payload
}

pub fn create_bitchat_message_payload_full(
    sender: &str,
    content: &str,
    channel: Option<&str>,
    is_private: bool,
    ...
    debug_full_println!(
        "[PAYLOAD] Creating message payload - sender: '{}', content: '{}', is_private: {}",
        sender,
        content,
        is_private
    );
    ...
    debug_full_println!(
        "[PAYLOAD] Flags: 0x{:02X} (is_private={}, has_sender_peer_id={})",
        flags,
        (flags & MSG_FLAG_IS_PRIVATE) != 0,
        (flags & MSG_FLAG_HAS_SENDER_PEER_ID) != 0
    );
    ...
    debug_full_println!(
        "[PAYLOAD] Created payload, total length: {}, message_id: {}",
        data.len(),
        id
    );
    debug_full_println!(
        "[PAYLOAD] Payload first 32 bytes: {:?}",
...
pub fn create_encrypted_channel_message_payload(
    ...
    let encrypted_content =
        match encryption_service.encrypt_with_key(content.as_bytes(), channel_key) {
            Ok(encrypted) => encrypted,
            Err(e) => {
                debug_println!("[!] Failed to encrypt message: {:?}", e);
                let (payload, id) = create_bitchat_message_payload_full(
                    sender,
                    content,
                    Some(channel),
                    false,
