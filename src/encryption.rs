...
impl EncryptionService {
    ...
    /// Current iOS derives the 8-byte routing peer ID from the SHA-256 hash of
    /// the announced Noise static public key.
    pub fn derive_peer_id(&self) -> String {
        use sha2::Digest;
    ...
    /// Public key advertised in the announce TLV as the Noise identity key.
    pub fn get_static_public_key_data(&self) -> Vec<u8> {
        self.public_key.as_bytes().to_vec()
    }

    /// Ed25519 public key advertised in the announce TLV for packet signatures.
    pub fn get_signing_public_key_data(&self) -> Vec<u8> {
        self.verifying_key.to_bytes().to_vec()
    }

    /// Add peer's public keys.
    ///
    /// The Rust client sends 96 bytes (X25519 + signing + identity), but other
    /// BitChat clients may send only the 32-byte X25519 key or omit the
    /// identity key. Keep the ECDH key exchange working even when the optional
    /// authentication keys are absent or malformed.
    pub fn add_peer_public_key(
        &self,
        peer_id: &str,
        public_key_data: &[u8],
    ) -> Result<(), EncryptionError> {
        if !matches!(public_key_data.len(), 32 | 64 | 96) {
            debug_println!(
                "[CRYPTO] Invalid public key data size: {}, expected 32, 64, or 96",
                public_key_data.len()
            );
            return Err(EncryptionError::InvalidPublicKey);
        }

        // The first 32 bytes are always the X25519 key agreement key.
        ...
        let signing_key = if public_key_data.len() >= 64 {
            let signing_key_bytes: [u8; 32] = public_key_data[32..64]
                .try_into()
                .map_err(|_| EncryptionError::InvalidPublicKey)?;

            match VerifyingKey::from_bytes(&signing_key_bytes) {
                Ok(key) => Some(key),
                Err(_) => {
                    debug_println!(
                        "[CRYPTO] Peer {} sent an invalid signing key; continuing with ECDH only",
                        peer_id
                    );
                    None
                }
            ...
            debug_println!(
                "[CRYPTO] Peer {} sent X25519-only key exchange data; continuing without signing key",
                peer_id
            );
            None
        };

        let identity_key = if public_key_data.len() >= 96 {
            let identity_key_bytes: [u8; 32] = public_key_data[64..96]
                .try_into()
                .map_err(|_| EncryptionError::InvalidPublicKey)?;

            match VerifyingKey::from_bytes(&identity_key_bytes) {
                Ok(key) => Some(key),
                Err(_) => {
                    debug_println!(
                        "[CRYPTO] Peer {} sent an invalid identity key; continuing without identity key",
                        peer_id
                    );
                    None
                }
        ...
        // Store the keys that were present and valid.
        {
    ...
    pub fn verify(
        ...
        let verifying_key = signing_keys
            .get(peer_id)
            .ok_or(EncryptionError::NoSharedSecret)?;
...
mod tests {
    ...
    #[test]
    fn test_key_exchange_accepts_x25519_only_payload() {
        let alice = EncryptionService::new();
        let bob = EncryptionService::new();
        let bob_x25519_key = bob.get_combined_public_key_data()[..32].to_vec();

        alice.add_peer_public_key("bob", &bob_x25519_key).unwrap();

        assert!(alice.has_peer_key("bob"));
    }

    #[test]
    fn test_key_exchange_accepts_invalid_optional_authentication_keys() {
        let alice = EncryptionService::new();
        let bob = EncryptionService::new();
        let mut bob_keys = bob.get_combined_public_key_data();
        bob_keys[32..64].copy_from_slice(&[0xff; 32]);
        bob_keys[64..96].copy_from_slice(&[0xff; 32]);
