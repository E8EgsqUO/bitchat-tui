...
pub fn decompress(data: &[u8]) -> Result<Vec<u8>, String> {
    decompress_size_prepended(data).map_err(|e| format!("Decompression failed: {}", e))
}

pub fn decompress_zlib(data: &[u8], output_size: usize) -> Result<Vec<u8>, String> {
    let decompressed = miniz_oxide::inflate::decompress_to_vec_zlib_with_limit(data, output_size)
        .or_else(|_| miniz_oxide::inflate::decompress_to_vec_with_limit(data, output_size))
        .map_err(|e| format!("Zlib/deflate decompression failed: {:?}", e))?;

    if decompressed.len() != output_size {
        return Err(format!(
            "Zlib decompressed size mismatch: got {}, expected {}",
            decompressed.len(),
            output_size
        ));
    }
...
pub fn decompress_raw(data: &[u8], output_size: usize) -> Result<Vec<u8>, String> {
    ...
    if data.len() > 4 && &data[0..4] == b"bv41" {
        ...
        if data.len() > 16 {
            ...
            debug_full_println!(
                "[DECOMPRESS] bv41 header: original_size={}, compressed_size={}",
                original_size_le,
                compressed_size_le
            );
            ...
            if data.len() >= 12 + compressed_size_le {
                ...
                debug_full_println!(
                    "[DECOMPRESS] Attempting to decompress {} bytes of LZ4 data",
                    compressed_size_le
                );
                debug_full_println!(
                    "[DECOMPRESS] First 32 bytes of LZ4 data: {:02x?}",
                ...
                if lz4_data.len() > 4 {
                    ...
                    if let Ok(result) = lz4_flex::decompress(&lz4_data[4..], output_size) {
                        debug_full_println!(
                            "[DECOMPRESS] Successfully decompressed after skipping 4-byte header"
                        );
                        return Ok(result);
                    }
                    ...
                    if let Ok(result) = lz4_flex::decompress(&lz4_data[2..], output_size) {
                        debug_full_println!(
                            "[DECOMPRESS] Successfully decompressed after skipping 2-byte header"
                        );
                        return Ok(result);
                    }
                }
        ...
        return Err(
            "Failed to decompress Apple bv41 format - could not find LZ4 data start".to_string(),
        );
    }
    ...
    for skip in [4, 8, 16].iter() {
        if data.len() > *skip {
            if let Ok(result) = lz4_flex::decompress(&data[*skip..], output_size) {
                debug_full_println!(
                    "[DECOMPRESS] Successfully decompressed with {}-byte header skip",
                    skip
                );
                return Ok(result);
            }
    ...
    Err(format!(
        "Raw decompression failed after trying multiple formats. Data len: {}, expected output: {}",
        data.len(),
        output_size
    ))
}
