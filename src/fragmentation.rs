use crate::{
    debug_full_println, packet_creation::create_bitchat_packet, parse_bitchat_packet, MessageType,
};
...
pub async fn send_packet_with_fragmentation_as(
    ...
    if packet.len() > 500 {
        debug_full_println!(
            "[DEBUG] ==================== FRAGMENTATION START ===================="
        );
        debug_full_println!("[DEBUG] Original packet size: {} bytes", packet.len());
        debug_full_println!(
            "[DEBUG] Original packet hex (first 64 bytes): {}",
            hex::encode(&packet[..std::cmp::min(64, packet.len())])
        );
        ...
        for (index, chunk) in chunks.iter().enumerate() {
            ...
            debug_full_println!(
                ...
                hex::encode(&chunk[..std::cmp::min(32, chunk.len())])
            );
            ...
            debug_full_println!(
                "[DEBUG] Fragment header: ID={} index={:02X}{:02X} total={:02X}{:02X} type={:02X}",
            ...
            debug_full_println!(
                "[DEBUG] Fragment payload hex: {}",
                hex::encode(&fragment_payload)
            );
            debug_full_println!("[DEBUG] Fragment payload breakdown:");
            debug_full_println!(
                "[DEBUG]   Fragment ID (8 bytes): {}",
                hex::encode(&fragment_payload[0..8])
            );
            debug_full_println!(
                "[DEBUG]   Index (2 bytes): {} = {:02X}{:02X}",
                index,
                fragment_payload[8],
                fragment_payload[9]
            );
            debug_full_println!(
                "[DEBUG]   Total (2 bytes): {} = {:02X}{:02X}",
                total_fragments,
                fragment_payload[10],
                fragment_payload[11]
            );
            ...
            debug_full_println!(
                "[DEBUG]   Data ({} bytes): {}",
                chunk.len(),
                hex::encode(&fragment_payload[13..std::cmp::min(13 + 32, fragment_payload.len())])
            );
            ...
            debug_full_println!(
                "[DEBUG] Final packet hex (first 64 bytes): {}",
                hex::encode(&fragment_packet[..std::cmp::min(64, fragment_packet.len())])
            );
            ...
            if let Ok(parsed_packet) = parse_bitchat_packet(&fragment_packet) {
                ...
                debug_full_println!(
                    "[DEBUG] Packet payload size: {} bytes",
                    parsed_packet.payload.len()
                );
                debug_full_println!(
                    "[DEBUG] Packet payload hex: {}",
                    hex::encode(&parsed_packet.payload)
                );

                // Verify the fragment payload structure
                if parsed_packet.payload.len() >= 13 {
                    let frag_id = &parsed_packet.payload[0..8];
                    let frag_index = ((parsed_packet.payload[8] as u16) << 8)
                        | (parsed_packet.payload[9] as u16);
                    let frag_total = ((parsed_packet.payload[10] as u16) << 8)
                        | (parsed_packet.payload[11] as u16);
            ...
            {
                return Err(format!(
                    "Failed to send fragment {}/{} (size: {} bytes)",
                    index + 1,
                    total_fragments,
                    fragment_packet.len()
                )
